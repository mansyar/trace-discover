// tools/patterns-compose.mjs — patterns-pack finals: turns the cutouts into the
// shipped shapes: goal arts (256, transparent like the shapes/letters arts),
// sticker seals (white disc + navy ring + object, composed at 520 and
// downscaled to 160), pack badge (400) and the patterns menu card (one reward
// object per motif in a row: yo-yo (loop) · snail (spiral) · step ladder
// (stairs), mirroring the shapes card). Reads art-src/patterns/clean, writes
// art-src/patterns/out + shipped WebP into public/art/{goal,sticker,pack}.
// Object-per-motif mapping (spec FR6, locked at art review):
// pattern-1 yoyo · pattern-2 hoop · pattern-3 rope (loops) · pattern-4 snail ·
// pattern-5 pinwheel · pattern-6 rose (spirals) · pattern-7 ladder ·
// pattern-8 slide · pattern-9 treehouse (stairs).
// usage (any cwd): node dev/tools/patterns-compose.mjs
...
const PATTERNS = path.resolve(HERE, '..', 'art-src', 'patterns');
const CLEAN = path.join(PATTERNS, 'clean');
const OUT = path.join(PATTERNS, 'out');
fs.mkdirSync(OUT, { recursive: true });

const OBJECTS = ['yoyo', 'hoop', 'rope', 'snail', 'pinwheel', 'rose', 'ladder', 'slide', 'treehouse'];

const jobs = [];
for (let i = 0; i < OBJECTS.length; i++) {
  const level = `pattern-${i + 1}`;
  jobs.push({ out: `goal-${level}.png`, kind: 'goal', src: [`obj-${OBJECTS[i]}.png`] });
  jobs.push({ out: `sticker-${level}.png`, kind: 'sticker', src: [`obj-${OBJECTS[i]}.png`] });
}
jobs.push({ out: 'patterns-badge.png', kind: 'badge', src: ['badge.png'] });
jobs.push({
  out: 'card-patterns.png',
  kind: 'card',
  src: ['obj-yoyo.png', 'obj-snail.png', 'obj-ladder.png'],
});
...
for (const job of jobs) {
  ...
  console.log(`ok art-src/patterns/out/${job.out} + public/art/${shipped.cls}/${path.basename(shipped.path)}`);
}
await browser.close();
