...
// Smoke: trex.riv loads in the real app runtime (canvas-lite) and its celebrate
// trigger fires through a fully traced level. Usage: node dev/qa/qa-trex.mjs
// (dev server on :5199 -- PORT env override; trex.riv shipped at public/rive/trex.riv).
const HERE = path.dirname(fileURLToPath(import.meta.url));
const BASE = `http://localhost:${process.env.PORT ?? 5199}`;
const OUT = path.join(HERE, 'out', 'trex');
...
await page.goto(`${BASE}/dev/harness/play.html?level=pre-2&char=trex`, { waitUntil: 'load' });
...
try {
  ...
  logs.push('trex pre-2: TRACE SUCCESS');
} catch {
  const state = await page.evaluate(
    () => document.getElementById('log').textContent.replace(/\n/g, ' | '),
  );
  logs.push(`trex pre-2: TRACE FAILED -- ${state}`);
}
