...
// Trex art-batch screenshots on the real app: menu / pack / level / success /
// parent with the trex skin seeded (backdrop + character + face icon).
// Usage: dev server running, then
// `node dev/qa/qa-trex-screens.mjs [url]`.
const HERE = path.dirname(fileURLToPath(import.meta.url));
const BASE = process.argv[2] ?? 'http://127.0.0.1:5299';
const OUT = path.join(HERE, 'out', 'trex-art');
...
// Spot-sweep: the skin cycle must reach trex (5 taps from the dino default).
for (let i = 0; i < 5; i += 1) {
...
if (cycled !== 'trex') {
  throw new Error(`skin cycle did not reach trex (got ${cycled})`);
}
...
// Parent zone with trex selected: the face icon on the small skin setter.
await page.reload({ waitUntil: 'load' });
await page.waitForFunction(() => window.__app && window.__app.screen, null, { timeout: 30000 });
await wait(800);
await tapTarget('splash');
