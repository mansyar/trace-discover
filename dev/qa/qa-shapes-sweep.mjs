...
// Shapes sweep: traces all 8 shape levels in one chain through the real app
// (menu -> pack grid -> level -> success -> next). Evidence for the shapes
// pack being completable end to end. Delete after use.
...
await tap('splash');
await tap('pack:shapes');
await tap('level:shape-1');
const done = [];
...
console.log(
  `cleared: ${save.completedLevels.filter((id) => id.startsWith('shape-')).length}, badges: ${JSON.stringify(save.badges)}`,
);
