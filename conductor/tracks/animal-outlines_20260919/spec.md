...
- **Outlines:** anatomy-driven stroke structure — one flowing closed outline where the silhouette is clean (duck, turtle, bunny, cat, elephant), 2–3 strokes where nature asks (fish body + tail/fin; ladybug dome + head line; butterfly mirrored wings + body). Each stroke keeps its own start star (per-stroke frontier — star/plus precedent). Exact stroke counts and control points are locked in `dev/harness/pack.html` with owner approval during authoring.
