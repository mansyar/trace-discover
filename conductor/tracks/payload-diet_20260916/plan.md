...
- [ ] Task: Re-encode tool (`dev/tools/reencode-art.mjs`) — best tracked source per asset (`dev/art-src` composite/cutout preferred; shipped file fallback), per-class quality (cutouts ≈0.85 lossy-with-alpha, backdrops ≈0.8); writes `.webp` + per-class side-by-side contact sheets into ignored `dev/qa/out/` (mechanical verification: deterministic re-run, sheet review)
- [ ] Task: Art-reference test (TDD: Red first) — every app-referenced art URL resolves to an existing `public/` file with the expected extension; **Red** while legacy `.png`/`.jpg` refs or files remain; hostile case (missing file) must fail
