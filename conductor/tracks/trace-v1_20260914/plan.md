...
- [ ] Task: Backdrop + goal art integration (Phase 5 miss: `bg-*`/`goal-*` art exists only in `spike/gen/`; no renderer draws them). Optimize + copy to `public/`, extend level/theme data with art refs, render behind the trail + at the goal endpoint, screenshot-verify per theme
