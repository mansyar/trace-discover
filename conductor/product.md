...
*2026-09-17 — Declarative pack pipeline (track `pack-pipeline_20260917`, in progress on branch `track/pack-pipeline`, no child-visible change): pack content becomes data — the Pre-writing pack ports to a validated JSON file (identical geometry), so new levels and packs are authored as data with schema-checked structure instead of hand-written code. The pack JSON is checked twice — at author time by a dev CLI (`pnpm pack:check`) and at load time by a shared parser — and a dev harness previews JSON levels for visual iteration. This closes the vision's "new content is pipeline work, not code" claim for the content axis (the art axis closed with the skins pipeline). Numbers, Letters and My Name follow on the same pipeline later.*
