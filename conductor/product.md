...
*2026-09-17 — Landscape layout (track `landscape-layout_20260917`) in progress on branch `track/landscape-layout`: a native landscape design space (860×430) beside the portrait field — every child screen re-lays for the wide box (portrait stays visually unchanged); word-class tracing (My Name, `ABC`/`MOM`/`ZOO`) recomposes at full letter size (5-letter names at full scale; 7-letter ≈2.3× today's portrait scale); rotation re-flows instantly with tracing progress preserved. Documented before implementation per `workflow.md`.*
...
- **Portrait + landscape play** (track `landscape-layout_20260917`, in progress): every screen re-lays for the wide field — an 860×430 landscape design space beside the portrait 430×860 — so word tracing (My Name, `ABC`/`MOM`/`ZOO`) composes at full letter size, and rotation re-flows instantly with progress kept
