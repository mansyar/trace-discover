...
**AI AGENT INSTRUCTION: This section is adapted to this project's specific language, framework, and build tools (Vite + TypeScript + pnpm + Biome). Keep it in sync with `tech-stack.md`.**
...
```bash
pnpm dev        # Vite dev server (desktop browser)
pnpm serve      # LAN static server for real-device testing (Android phone / iPad)
pnpm test       # Vitest run (logic/unit tests)
pnpm check      # Biome lint + format check, then tsc --noEmit
```

### Before Committing

```bash
# Run all pre-commit checks, non-interactively
CI=true pnpm check && CI=true pnpm test
```
